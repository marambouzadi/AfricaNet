...
import { UploadCloud, CheckCircle2, Laptop, HardDrive, Smartphone, ChevronRight, ChevronLeft, AlertCircle, X, Loader2 } from 'lucide-react'
                ...
                {step === 2 && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="pt-4 space-y-3">
                            ...
                            <label className="border-2 border-dashed border-[#E2E2DF] rounded-xl p-8 text-center hover:bg-gray-50 transition-colors cursor-pointer block relative">
                                <input type="file" accept="image/*" multiple onChange={handleFileUpload} className="hidden" disabled={isUploading} />
                                {isUploading ? (
                                    <Loader2 className="h-8 w-8 text-[#1A3FA0] mx-auto mb-3 animate-spin" />
                                ) : (
                                    <UploadCloud className="h-8 w-8 text-[#1A3FA0] mx-auto mb-3" />
                                )}
                                <p className="font-medium text-[#1A1A1A]">
                                    {isUploading ? 'Téléversement des photos en cours...' : 'Cliquez ou glissez vos photos ici'}
                                </p>
                            ...
                            {uploadedImages.length > 0 && (
                                <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 mt-4">
                                    {uploadedImages.map((imgUrl, idx) => (
                                        <div key={idx} className="relative group w-full h-24 rounded-lg overflow-hidden border border-[#E2E2DF]">
