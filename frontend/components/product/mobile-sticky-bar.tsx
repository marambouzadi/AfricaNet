  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 flex items-center justify-between border-t border-[#E2E2DF] bg-white px-4 py-3 shadow-lg md:hidden">
      ...
      <button
        type="button"
        disabled={isOut}
        onClick={() => addItem({ id: product.id, name: product.name, price: product.priceNum, image: product.thumbnails?.[0], stock: product.stock, condition: product.condition })}
        className="rounded-lg bg-[#1A3FA0] px-6 py-2.5 font-medium text-white transition-colors duration-200 hover:bg-[#0D2660] disabled:bg-[#9CA3AF] disabled:cursor-not-allowed"
      >
